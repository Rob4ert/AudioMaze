...
export default function Fake(props: any) {
  ...
  let counter = 0;
  const addPodcast = podcasts.results?.map((el: any) => {
    counter++;
    return (
      <li id="each">
        ...
        <div id="logodiv">
          <a id="podId">
            <img id="logo" src={el.image} alt="poster" onClick={taker}></img>
          </a>
        </div>
        <div id="rank-place">
          <a id="podId" onClick={taker}>
            <p className="title-p">{[el.title_original]}</p>
          </a>
        </div>
        <div id="rating">
          <div className="Fafa">
            ...
            <Fav
              liked={props.liked}
              setLiked={(i: any) => props.setLiked(i)}
              title={el.title}
              ids={el.id}
            />
          </div>
  ...
  return (
    <>
      ...
      <div id="rank">
        ...
        <ul className="list">{addPodcast}</ul>
      </div>
    </>
  );
}
